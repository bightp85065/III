        ...
        function start(){
            ...
            if($.needImageZoom){
                ...
                if( $(window).height()<350 ){
                    //最小縮成到400
                    if($.drawImg.width>400&& $.drawImg.height>400){
                        $.imgZoom = $.drawImg.width<$.drawImg.height ? $.drawImg.width/400 : $.drawImg.height/400;
                    }
                }
                else{
                    var widthZoom = $.drawImg.width/$(window).width();
                    var heightZoom = $.drawImg.height/($(window).height()-350);

                    if(widthZoom>1||heightZoom>1){
                        $.imgZoom = widthZoom>heightZoom ? widthZoom : heightZoom;
                    }
                }
                $.imgZoom = GetDecimals( 1/$.imgZoom , 2 );
            }
            
            $.drawImg.width *= $.imgZoom;
            $.drawImg.height *= $.imgZoom;
...
//            $(".dragPlace").mousemove(function(e){handleMouseMoveForCover(e);});
            //bohan mark
...
//            drawTheImage($.drawImg,$.drawImageOpacity);
        ...
        function drawWholeForCalPixel(){
            $.drawCtx.clearRect(0,0,$.drawCanvas.width,$.drawCanvas.height);
            
            for(var i=0;i<$.drawRegionClassifications.length;i++){
                drawRegionClassificationForCalPixel($.drawRegionClassifications[i]);
            }
        }
...
//            $.drawCtx.shadowColor = "gray";  
        ...
        function drawRegionClassificationForCalPixel(regionClassifications){
            if(regionClassifications===undefined){return;}
...
//            console.log("handleMouseUp");
            ...
            if($("#btnRequirementArea").hasClass("active")){
                ...
                if($.imageScale!==0&&$.scalePixel!==undefined)
                ...
                addOneSensorNode($.drawMouseX,$.drawMouseY,0,$.drawSensorNode.length,opt.attr("image"),opt.attr("device"));
...
//            console.log("handleMouseUp");
        ...
        function handleMouseMoveForCover(e){
            if(!$.indr_RSSI_information ){return;}
...
//            var originData;
        ...
        function addOneUplinkPoint(x, y, height, index, image, device){
                ...
                drawWhole();
//                appendUplinkPointHtmlToOperateList();
        ...
        function appendUplinkPointHtmlToOperateList(){
            
        }
        
        function addOneSensorNode(x, y, height, index, image, device){
                var infoData = { x:x,y:y,height:height ,index:index,image:image};
...
//                dragPlaceEvent();
        ...
        function getRequirementAreaPixel(wallArr, requirementAreaMinMax){
            
            var tmp = [];
            tmp[0] = $.drawLines;
            tmp[1] = $.drawStraightLines;
            tmp[2] = $.drawSquares;
            tmp[3] = $.drawSquaresForElevator;
            tmp[4] = $.drawUplinkPoint;
            tmp[5] = $.drawSensorNode;
            tmp[6] = $.drawRegionClassifications;
            tmp[7] = $.drawRegionClassificationsTemp;
            tmp[8] = $.drawRequirementAreaPolygonals;
            tmp[9] = $.drawRequirementAreaPolygonalsTemp;
            tmp[10] = $.imageScale;
            $.debugTmp = tmp;
            initParameters()
            drawWhole();
            
            $.drawRequirementAreaPolygonals = tmp[8];
            drawWhole();
            
//            var requirementArea = [];
            var imageData = $.drawCtx.getImageData(0, 0, $.drawImg.width-1, $.drawImg.height-1);
            
            for(var x=requirementAreaMinMax.x1-1;x<requirementAreaMinMax.x2;++x){
//                requirementArea[x] = [];
                for(var y=requirementAreaMinMax.y1-1;y<requirementAreaMinMax.y2;++y){
...
//                    requirementArea[x][y] = (r!==0&&g!==0&&b!==0) ? 1 : 0;
                    if(r==0&&g==0&&b==0){
                        wallArr[wallArr.length] = {"x":x,"y":y,"w_type":"0"};
                    }
...
//                    }
            ...
            $.drawLines = tmp[0];
            $.drawStraightLines = tmp[1];
            $.drawSquares = tmp[2];
            $.drawSquaresForElevator = tmp[3];
            $.drawUplinkPoint = tmp[4];
            $.drawSensorNode = tmp[5];
            $.drawRegionClassifications = tmp[6];
            $.drawRegionClassificationsTemp = tmp[7];
            $.drawRequirementAreaPolygonals = tmp[8];
            $.drawRequirementAreaPolygonalsTemp = tmp[9];
            $.imageScale = tmp[10];
            drawWhole();
        ...
        function getAllRegionClassificationPixel(){
            ...
            $.drawRegionClassifications = tmp[6];
            drawWholeForCalPixel();
            
            var regionClassifications = [];
            var imageData = $.drawCtx.getImageData(0, 0, $.drawImg.width-1, $.drawImg.height-1);
            
            var rr,gg,bb,rc_arr = {};
            $.each($.region_classification,function(k,v){
                
                rr = v.rc_color_rgb.substr(0, 2);
                gg = v.rc_color_rgb.substr(2, 2);
                bb = v.rc_color_rgb.substr(4, 2);
                
                var value = (parseInt(rr,16)).toString()
                            + (parseInt(gg,16)).toString()
                            + (parseInt(bb,16)).toString();
                rc_arr[value] = v.rc_attenuation_factor;
                
            });
            console.log(rc_arr);
            var debugR, debugG, debugB, r, g, b, a, index;
            for(var x=0;x<$.drawImg.width-1;++x){
                for(var y=0;y<$.drawImg.height-1;++y){
                    ...
                    if(r!==0||g!==0||b!==0){
                        var tmpRGB = r.toString()+ g.toString()+ b.toString();
                        if(rc_arr[tmpRGB]!==undefined)
                            regionClassifications.push( {"x":x, "y":y, "rc_value":GetDecimals(1/rc_arr[tmpRGB],2).toString()} );
//                            console.log("(x,y)=("+x+","+y+"),index="+index+",r="+r+",g="+g+",b="+b+",a="+a);
//                        if(debugR==r&& debugG==g&& debugB==b)
//                            console.log("r="+r+",g="+g+",b="+b+",a="+a);
                        debugR=r; debugG=g; debugB=b;
                    }
                }
            }
...
//                                            "x":50,"y":54,"rc_value":"1.33"}]
        ...
        $(".dragPlace").delegate('[component-type="sensorNode"]',"contextmenu",function(){
            console.log("uplinkPoint click")
            if( $.sensorNodeRSSI!== undefined ){
                var sensorNodeX = parseInt($(this).css("left"))+15;
                var sensorNodeY = parseInt($(this).css("top"))+15;
                console.log(sensorNodeX, sensorNodeY);
                $.each($.sensorNodeRSSI,function(k,v){
                    if( v.x==sensorNodeX && v.y==sensorNodeY ){
                        swal("SensorNode RSSI Value", "RSSI Power = "+v.rssi+" dbm");
                        return false;
                    }
                });
            }
